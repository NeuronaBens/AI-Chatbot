...
export async function POST(req, { params }) {
  ...
  // Calculate the time difference between the current date and the date of the last message
  const currentDate = new Date();
  const lastMessageDate = firstMessageOfLastSession
    ? new Date(firstMessageOfLastSession.date_send)
    : null;
  const timeDifference = lastMessageDate
    ? (currentDate - lastMessageDate) / (1000 * 60 * 60 * 24) // Convert milliseconds to days
    : null;

  // Check if a week has passed since the last message
  if (timeDifference && timeDifference >= 7) {

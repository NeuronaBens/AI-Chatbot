...
const getStudentTasks= async (student_id)=> {
  const {data, error} = await supabase.from('StudentTask').
...
const processTask = async (completion, student_id) => {
  ...
  if(existingTaskIds.length > 0){
    matchingTask = tasks.find(
      (task) => completion.toLowerCase().includes(task.name.toLowerCase()) && !existingTaskIds.includes(task.id)
    );
  }
  else{
    matchingTask = tasks.find(
      (task) => completion.toLowerCase().includes(task.name.toLowerCase())
    );
  }
  
  if (matchingTask){
    const newStuTask = await createStudentTask(student_id, matchingTask.id);
    console.log(newStuTask);
  }
}
